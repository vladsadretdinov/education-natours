...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 2) Check of user exist && password is correct
  ...
  // 3) If everything onkeydown, send token to the client
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError("You don't have permissions to perfom this action", 403)
      );
    }
...
exports.forgetPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: 'Your passwor reset token (valid for 10 min)',
      message,
    });
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  // User.findByIdAndUpdate() will NOT work as intendent
